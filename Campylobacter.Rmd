...
annotations <- read.delim("~/bigfiles/eproNOG.annotations.tsv", header=F)
genes <- vector()
...
```{r, does PCA do a good job of separating invasive and gastrointestinal strains using our top indicators?}
pcadata <- na.roughfix(traindata[,bestfeatures])
ir.pca <- prcomp(pcadata,center = TRUE, scale=TRUE) 

library(rgl)
plot3d(ir.pca$x[,1:3], col=as.numeric(factor(traindata$class)))

```

PCA doesn't do a much better job that MreB and Pgp1 in separating invasive and gastrointestnal strains

```{r, evaluating significance of DBS values}
dbs_indiv <- c(data2)
dbs_indiv2 <- unlist(dbs_indiv)
dbs_indiv2 <- dbs_indiv2[!is.na(dbs_indiv2)]

sum(abs(dbs_indiv2)>2.5)/length(dbs_indiv2)
```
